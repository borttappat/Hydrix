...
enable_headphones() {
    ...
    # Boost input gain to max (+16dB)
    amixer -c $CARD_NUM cset name='cs42l43 Headphone L Input 1 Volume' 48 >/dev/null
    amixer -c $CARD_NUM cset name='cs42l43 Headphone R Input 1 Volume' 48 >/dev/null

    wpctl set-default $HEADPHONE_DEVICE >/dev/null 2>&1
    wpctl set-volume $HEADPHONE_DEVICE 1.5 >/dev/null 2>&1
