...
# Generate local/host.nix
generate_local_host() {
    local output_dir="$1"
    local username="$2"

    mkdir -p "$output_dir"
    local host_file="$output_dir/host.nix"

    cat > "$host_file" << EOF
# Host configuration - Generated by Hydrix installer
#
# This file contains ONLY the username.
# Password is your system password (set during installation).
{
  username = "$username";
}
EOF
    ...
    chmod 600 "$host_file"
    log "Generated: $host_file"
}

# Generate local/shared.nix
generate_local_shared() {
    local output_dir="$1"
    local username="$2"
    local timezone="${3:-$(detect_timezone)}"
    local locale="${4:-$(detect_locale)}"
    local console_keymap="${5:-$(detect_console_keymap)}"
    local xkb_layout="${6:-$(detect_xkb_layout)}"
    local xkb_variant="${7:-$(detect_xkb_variant)}"

    mkdir -p "$output_dir"
    local shared_file="$output_dir/shared.nix"

    cat > "$shared_file" << EOF
# Shared non-secret configuration
# Auto-generated by Hydrix installer
{
...
EOF
    ...
    chmod 644 "$shared_file"
    log "Generated: $shared_file"
}

# Generate local/router.nix
generate_local_router() {
    ...
    mkdir -p "$output_dir"
    local router_file="$output_dir/router.nix"

    cat > "$router_file" << EOF
# Router VM credentials - Generated by Hydrix installer
#
...
EOF
    ...
    log "Generated: $router_file"
}

# Generate local/vms/ directory structure with placeholders
generate_local_vms_structure() {
    local output_dir="$1"
    local vms_dir="$output_dir/vms"

    mkdir -p "$vms_dir"

    local vm_types=("pentest" "browsing" "lurking" "dev" "comms")
...
EOF
    ...
    log "Generated VM secrets structure in $vms_dir"
}
