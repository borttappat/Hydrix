...
check_existing_secrets() {
    # Check if secrets file already exists for this hostname
    local secrets_file="$VMS_DIR/${VM_HOSTNAME}.nix"

    if [[ -f "$secrets_file" ]]; then
        log "Found existing secrets for $VM_HOSTNAME"

        # Ask user if they want to reuse existing credentials
        echo ""
        read -p "Reuse existing credentials? [Y/n]: " -r reuse_choice
        reuse_choice=${reuse_choice,,}

        if [[ ! $reuse_choice =~ ^(n|no)$ ]]; then
            log "Reusing existing credentials"
            REUSE_CREDENTIALS=true

            # Extract existing values from secrets file
            VM_PASSWORD_HASH=$(grep 'hashedPassword' "$secrets_file" | sed 's/.*"\(.*\)".*/\1/')
            local existing_user=$(grep 'username' "$secrets_file" | sed 's/.*"\(.*\)".*/\1/')

            if [[ -n "$existing_user" ]]; then
                VM_USER="$existing_user"
            fi
...
prompt_password() {
    ...
    # Skip if reusing existing credentials
    if [[ "$REUSE_CREDENTIALS" == true ]]; then
        log "Using existing password hash"
        return 0
    fi
...
generate_password_hash() {
    # Skip if reusing existing credentials (hash already loaded)
    if [[ "$REUSE_CREDENTIALS" == true ]]; then
        return 0
    fi
...
generate_secrets_file() {
    # Skip if reusing existing credentials (file already exists)
    if [[ "$REUSE_CREDENTIALS" == true ]]; then
        log "Reusing existing secrets file"
        return 0
    fi
    ...
    # Note: No timestamps in nix files - they defeat nix's caching
    # Timestamps are stored in the JSON credentials reference instead
    cat > "$secrets_file" << EOF
# VM Secrets - LOCAL ONLY (not committed to git)
...
generate_instance_config() {
    ...
    # Note: No timestamps - they defeat nix's caching
    cat > "$instance_file" << EOF
# VM Instance Configuration (temporary, used during image build)
{
  hostname = "${VM_HOSTNAME}";
  username = "${VM_USER}";
}
...
main() {
  ...
  sudo nixos-rebuild switch --specialisation maximalism"
    ...
    # Check for existing secrets first (allows reuse for fast rebuilds)
    check_existing_secrets
    prompt_password
    generate_password_hash
    generate_secrets_file
    ...
    # Nix caching handles rebuild detection - if secrets unchanged, build is instant
