...
prompt_auth_method() {
    # All display output goes to stderr so it's visible when called as $(prompt_auth_method)
    echo "" >&2
    warn "Authentication required for private repository" >&2
    echo "" >&2
    echo "Options:" >&2
    ...
    echo "  3) SSH key" >&2
    echo "  4) Read token from file (e.g. USB drive)" >&2
    echo "  5) Try without authentication (public repo)" >&2
    echo "" >&2
...
authenticate_gh_cli() {
    ...
    if ! command_exists gh && [[ "$gh_cmd" == "gh" ]]; then
        if ! command_exists nix-shell; then
            warn "gh not installed and nix-shell not available — use option 2 (Personal Access Token)"
            return 1
        fi
        log "GitHub CLI not installed — loading temporarily via nix-shell..."
        echo ""
        echo "  Inside the gh prompt:"
        ...
        echo "    - Or select 'Paste an authentication token'"
        echo "      generate one at: github.com/settings/tokens  (scope: repo)"
        echo ""
        nix-shell -p gh --run "gh auth login --hostname github.com --git-protocol https"
        # Signal _get_gh_token to extract via the same nix-shell invocation
        _GH_VIA_NIX_SHELL=1
        return 0
    fi
...
_get_gh_token() {
    ...
    if [[ -n "${_GH_VIA_NIX_SHELL:-}" ]]; then
        # gh was loaded via nix-shell; auth config is in ~/.config/gh/hosts.yml
        token=$(nix-shell -p gh --run "gh auth token" 2>/dev/null) || true
    elif [[ -n "${SUDO_USER:-}" ]] && [[ "$SUDO_USER" != "root" ]]; then
        local user_home
        user_home=$(getent passwd "$SUDO_USER" | cut -d: -f6)
        # Pass explicit HOME so gh finds its config even under sudo
        token=$(HOME="$user_home" sudo -u "$SUDO_USER" gh auth token 2>/dev/null) || true
        if [[ -z "$token" ]] && [[ -f "$user_home/.config/gh/hosts.yml" ]]; then
            token=$(awk '/^github\.com:/{f=1} f && /oauth_token:/{print $2; exit}' \
                "$user_home/.config/gh/hosts.yml")
        fi
    else
        token=$(gh auth token 2>/dev/null) || true
    fi

    # Universal YAML fallback (covers root with no SUDO_USER and nix-shell failures)
    ...
    if [[ -z "$token" ]]; then
        echo "[WARN] Could not extract GitHub token automatically" >&2
        read -s -p "Paste your GitHub Personal Access Token (or press Enter to cancel): " token < /dev/tty
        echo "" >&2
    fi
    echo "$token"
}
...
try_clone_with_auth() {
    ...
    case "$auth_method" in
        1)
            # GitHub CLI — authenticate, then extract token for git under root
...
# /mnt/boot. Builds static GRUB entries for previous NixOS/Hydrix installs
# only — non-NixOS OSes (Windows, Ubuntu, etc.) are detected automatically
# by os-prober on every rebuild (boot.loader.grub.useOSProber = true).
#
# Plain previous NixOS/Hydrix installs (kernels already on the EFI):
...
_finalize_dual_boot_entries() {
    ...
    # Non-NixOS OSes (Windows, Ubuntu, Fedora, etc.) are detected automatically
    # by os-prober on every rebuild via boot.loader.grub.useOSProber = true.
    # No static EFI scan is needed or generated here for those OSes.
    ...
    case "${source_choice:-1}" in
        1)
            ...
            if [[ "$HYDRIX_BRANCH" == "main" ]]; then
                ...
                CONFIG[hydrixUrl]="github:borttappat/Hydrix/${HYDRIX_BRANCH}"
            fi
...
_installer_hydrix_source() {
    if [[ -n "$HYDRIX_CLONE_DIR" ]] && [[ -d "$HYDRIX_CLONE_DIR/Hydrix" ]]; then
        echo "$HYDRIX_CLONE_DIR/Hydrix"
    elif [[ -n "$SCRIPT_DIR" ]] && [[ -f "$(dirname "$SCRIPT_DIR")/flake.nix" ]]; then
        echo "$(dirname "$SCRIPT_DIR")"
    fi
}

# If the cloned flake.nix uses a local path: input for hydrix:
#   1. Rewrite the URL to the installer's available source so nix flake lock works
#   2. Set HYDRIX_LOCAL_TARGET to the intended path on the installed system
#      so install_nixos can copy Hydrix there and restore the URL.
handle_local_hydrix_path() {
    local config_dir="$1"
    ...
    # github:/git+https: URLs are good as-is — nothing to do
    [[ "$hydrix_url" == path:* ]] || [[ "$hydrix_url" == git+file://* ]] || return 0

    local original_path="${hydrix_url#path:}"
    original_path="${original_path#git+file://}"

    # path: URLs in a committed config should not reach the installer.
    # The recommended workflow is: flake.nix points to a github: URL; use
    # 'rebuild --dev' for local overrides without editing the file.
    warn "  flake.nix has a local path: URL ($hydrix_url)."
    warn "  This should be a remote URL (e.g. github:borttappat/Hydrix/sops-secrets)."
    warn "  Use 'rebuild --dev' for local overrides instead of editing flake.nix."
    echo ""
    echo "  Enter the remote URL to use during installation:"
    echo "  Examples:"
...
install_nixos() {
    ...
    # Write resume state — allows re-running the installer if nixos-install fails.
    # The state is read by check_resume() on next startup.
    cat > /mnt/hydrix-install.state <<EOF
RESUME_USERNAME="${CONFIG[username]}"
RESUME_SERIAL="${CONFIG[serial]}"
RESUME_HYDRIX_LOCAL_TARGET="${HYDRIX_LOCAL_TARGET:-}"
RESUME_MODE="${MODE}"
RESUME_DEVICE="${CONFIG[device]:-}"
EOF
    ...
    if [[ $total_gb -lt 32 ]]; then
        ...
        if [[ "$fstype" == "btrfs" ]]; then
            # btrfs requires COW disabled BEFORE writing — truncate first, chattr, then fill
        ...
        if [[ "$swap_ok" == true ]]; then
            ...
            mkswap /mnt/.swapfile
            swapon /mnt/.swapfile
            log "  Swapfile active (${swap_needed}GB on ${fstype}, will be removed after install)"
        else
    ...
    if [[ -n "$HYDRIX_LOCAL_TARGET" ]] && [[ ! -e "$HYDRIX_LOCAL_TARGET" ]]; then
        mkdir -p "$(dirname "$HYDRIX_LOCAL_TARGET")"
        ln -sf "/mnt$HYDRIX_LOCAL_TARGET" "$HYDRIX_LOCAL_TARGET"
        _hydrix_symlink="$HYDRIX_LOCAL_TARGET"
        log "Created temporary symlink $HYDRIX_LOCAL_TARGET → /mnt$HYDRIX_LOCAL_TARGET for prebuild"
    fi

    prebuild_microvms "$config_dir"

    [[ -n "$_hydrix_symlink" ]] && rm -f "$_hydrix_symlink"
}
...
check_resume() {
    # If /mnt is mounted and a state file exists from a previous run, offer to resume.
    # nixos-install is idempotent — safe to re-run against the same config.
    mountpoint -q /mnt 2>/dev/null || return 1
    [[ -f /mnt/hydrix-install.state ]] || return 1

    source /mnt/hydrix-install.state
    ...
    echo ""
    echo "  Username : $RESUME_USERNAME"
    echo "  Machine  : $RESUME_SERIAL"
    echo "  Config   : $config_dir"
    [[ -n "${RESUME_DEVICE:-}" ]] && echo "  Device   : $RESUME_DEVICE"
    echo ""
    read -p "Resume installation? [y/N]: " resume_yn < /dev/tty
    [[ "${resume_yn,,}" == y* ]] || return 1

    # Restore state into CONFIG so install_nixos and post-install steps work
    CONFIG[username]="$RESUME_USERNAME"
    CONFIG[serial]="$RESUME_SERIAL"
    MODE="${RESUME_MODE:-fresh}"
    HYDRIX_LOCAL_TARGET="${RESUME_HYDRIX_LOCAL_TARGET:-}"

    log "Resuming from nixos-install..."
    echo ""

    # nixos-install (config is already on /mnt)
    local mem_gb
    mem_gb=$(awk '/MemTotal/ {printf "%d", $2/1024/1024}' /proc/meminfo)
    local max_jobs=1 cores=2
    if (( mem_gb >= 32 )); then max_jobs=4; cores=4
    ...
    if [[ "$active_hydrix_url" == path:* ]] || [[ "$active_hydrix_url" == git+file://* ]]; then
        warn "  flake.nix has a local path: URL ($active_hydrix_url) — needs a remote URL for install."
        echo ""
        echo "  Enter the remote URL to use during installation:"
        echo "  Examples:"
        echo "    github:borttappat/Hydrix/sops-secrets"
    ...
    if [[ -n "$HYDRIX_LOCAL_TARGET" ]]; then
        ...
        (cd "$config_dir" && \
            git -c user.name="Hydrix Installer" -c user.email="installer@hydrix" add flake.nix && \
            git -c user.name="Hydrix Installer" -c user.email="installer@hydrix" commit --amend --no-edit)
    fi
    ...
    rm -f /mnt/hydrix-install.state
    echo ""
    success "Installation resumed and completed successfully."
    exit 0
}
...
access-tokens = github.com=$gh_token"
    ...
    # Cleanup temp directories and resume state (install succeeded)
    cleanup_temp_config
