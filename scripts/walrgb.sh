...
# Force update i3wm.color4 in Xresources to override any daemon-set value
# This ensures i3 picks up the new wal color instead of the stale daemon color
if [ -f ~/.cache/wal/colors.json ]; then
    COLOR4=$(jq -r '.colors.color4' ~/.cache/wal/colors.json)
    xrdb -merge <<< "i3wm.color4: $COLOR4"
fi

# Force i3 to reload colors (preserve workspace focus)
