...
#   - Specialisations for fallback (re-enable WiFi) and lockdown (VPN isolation)
#
# Modes:
#   - Base (default boot): Router VM with 192.168.x.x (simple NAT)
#   - Fallback specialisation: Re-enables WiFi, normal NetworkManager
#   - Lockdown specialisation: 10.100.x.x with VPN policy routing, host isolated
...
{ config, lib, pkgs, ... }:
{
  ...
  systemd.services.router-vm-autostart = {
    ...
    script = ''
      ...
      if ! /run/current-system/sw/bin/virsh dominfo "\$VM_NAME" >/dev/null 2>&1; then
        ...
        # NOTE: Network interfaces MUST come before hostdev to ensure
        # virtio NICs get lower PCI slots (enp1s0, enp2s0, etc.)
        # WiFi passthrough will be named wlp* (wireless naming convention)
  ...
  # Inherits VFIO and bridges from base config, just changes IPs and adds isolation
  specialisation.lockdown.configuration = {
    ...
    # Lockdown router VM autostart (uses different VM name for lockdown mode)
