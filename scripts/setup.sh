...
update_users_nix() {
    local username="$1"
    local users_file="$PROJECT_DIR/modules/base/users.nix"

    # If user is traum, no changes needed (already configured)
    if [[ "$username" == "traum" ]]; then
        log "User is 'traum' - users.nix already configured"
        return
    fi

    log "Updating users.nix for user: $username"

    if [[ ! -f "$users_file" ]]; then
        error "users.nix not found at: $users_file"
    fi

    # Get user's full name from passwd (or use username as fallback)
    local user_fullname
    ...
    # Replace 'traum' with the detected username throughout the file
    sed -i "s/users\.users\.traum/users.users.$username/g" "$users_file"
    sed -i "s/autologinUser = \"traum\"/autologinUser = \"$username\"/g" "$users_file"
    sed -i "s/users = \[ \"traum\" \]/users = [ \"$username\" ]/g" "$users_file"
    sed -i "s/password = \"traum\"/password = \"$username\"/g" "$users_file"
    sed -i "s/# Define user 'traum'/# Define user '$username'/g" "$users_file"
    sed -i "s/description = \"A\"/description = \"$user_fullname\"/g" "$users_file"

    success "users.nix updated for user: $username"
}
...
show_completion_summary() {
    ...
    echo "  [+] modules/base/users.nix (user: $username)"
