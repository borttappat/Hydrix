...
generate_user_config() {
    local username="$1"
    local user_config_path="$GENERATED_DIR/user.nix"

    log "Generating user configuration for: $username"
    ...
    local shell_pkg="pkgs.bash"  # default
    case "$user_shell" in
        */fish) shell_pkg="pkgs.fish" ;;
        */zsh)  shell_pkg="pkgs.zsh" ;;
        */bash) shell_pkg="pkgs.bash" ;;
    esac

    cat > "$user_config_path" << USEREOF
# User configuration - Auto-generated by setup.sh
# Generated on: $(date)
#
# This file defines the primary user for this machine.
# It is imported by the machine profile and VM configurations.
...

{
  ...
  users.users.$username = {
    ...
    extraGroups = [ "docker" "audio" "networkmanager" "wheel" "wireshark" "adbusers" "libvirtd" "kvm" ];
    createHome = true;
    shell = $shell_pkg;
  ...
  # Home directory for user-specific configs
...
USEREOF
    ...
    success "User configuration created: $user_config_path"
}
...
show_completion_summary() {
    ...
    echo "  [+] generated/user.nix (user: $username)"
...
This script automatically:
  1. Detects hostname, CPU platform (Intel/AMD), and current user
  2. Creates machine profile from template with VFIO/specialisations
  3. Generates user configuration (generated/user.nix)
  4. Updates flake.nix with new machine entry
