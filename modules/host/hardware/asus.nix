    ...
    set_mode() {
      ...
      case "$mode" in
        ...
        balanced|auto)
          ...
          for epp in /sys/devices/system/cpu/cpu*/cpufreq/energy_performance_preference; do
            echo balance_power | sudo tee "$epp" > /dev/null 2>&1 || true
          done
          if command -v asusctl >/dev/null 2>&1; then
...
in {
  config = lib.mkIf cfg.hardware.isAsus {
    ...
    # Apply battery charge thresholds at boot and on charger hotplug
    # (from hydrix.power.chargeLimit / chargeStartLimit)
    systemd.services.battery-charge-limit = lib.mkIf (config.hydrix.power.chargeLimit != null) {
      ...
      script = let
        endLimit = toString config.hydrix.power.chargeLimit;
        startLimit = lib.optionalString
          (config.hydrix.power.chargeStartLimit != null)
          (toString config.hydrix.power.chargeStartLimit);
      in ''
        for bat in /sys/class/power_supply/BAT*; do
          if [ -f "$bat/charge_control_end_threshold" ]; then
            echo ${endLimit} > "$bat/charge_control_end_threshold"
          fi
          ${lib.optionalString (config.hydrix.power.chargeStartLimit != null) ''
          if [ -f "$bat/charge_control_start_threshold" ]; then
            echo ${startLimit} > "$bat/charge_control_start_threshold"
          fi
          ''}
        done
      '';
    ...
    # Re-apply charge thresholds whenever a charger/USB-C supply is connected.
    # Without this, plugging in a charger mid-session resets thresholds to 100%.
    services.udev.extraRules = lib.mkIf (config.hydrix.power.chargeLimit != null) ''
      ACTION=="add|change", SUBSYSTEM=="power_supply", KERNEL!="BAT*", \
        RUN+="${pkgs.systemd}/bin/systemctl start battery-charge-limit.service"
    '';
