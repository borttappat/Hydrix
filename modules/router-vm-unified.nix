...
let
  ...
  # Load router credentials from local/router.nix (gitignored, generated by setup.sh)
  # Path is relative from modules/ to local/
  routerConfigPath = ../local/router.nix;
  routerConfig =
    if builtins.pathExists routerConfigPath
    then import routerConfigPath
    else {
      # Defaults if local/router.nix doesn't exist
      username = "user";
      hashedPassword = null;  # Will use password below as fallback
    };
...
in {
  ...
  config = {
    ...
    # Router VM user - credentials from local/router.nix (generated by setup.sh)
    # If local/router.nix doesn't exist, falls back to "user" with password "router"
