...
ZEOF
  ...
  # Takes a fresh screenshot - used for manual locking (Mod+Shift+e) and idle timeout
  lockScript = pkgs.writeShellScriptBin "lock" ''
    ...
    # Take a screenshot
    ${pkgs.scrot}/bin/scrot -o $img

    ${if cfg.lockscreen.blur then ''
    # Apply blur/pixelate effect
    ${pkgs.imagemagick}/bin/magick $img -scale 20% -scale 500% $blur_img
    '' else ''
    cp $img $blur_img
    ''}
    ...
    if ! ${pkgs.imagemagick}/bin/magick $blur_img -gravity NorthWest \
        -pointsize $FONT_SIZE -font "$FONT" -fill "$color1" \
        -annotate +50+50 "$LOCK_TEXT" $text_img 2>/dev/null; then
      FONT="CozetteVector"
      ${pkgs.imagemagick}/bin/magick $blur_img -gravity NorthWest \
          -pointsize $FONT_SIZE -font "$FONT" -fill "$color1" \
          -annotate +50+50 "$LOCK_TEXT" $text_img
    fi
  ...
  lockInstantScript = pkgs.writeShellScriptBin "lock-instant" ''
    ...
    if [ -f "$LOCK_CACHE" ]; then
      ...
      ${pkgs.imagemagick}/bin/magick -size 1920x1080 "xc:$color0" "$LOCK_IMG" 2>/dev/null || true
    fi
