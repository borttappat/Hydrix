...
#
# Workspace border colors are user-configurable via hydrix.hyprland.workspaceColors
# (attrset of workspace number strings to RGBA hex colors). Defaults to empty.
#
...
#
{
  ...
  gaps = toString (config.hydrix.graphical.ui.gaps or 10);
  borderSize = toString (sc.border_size or 2);
  rounding = toString (sc.corner_radius or 10);

  # Workspace border colors — user-configurable via hydrix.hyprland.workspaceColors.
  # Defaults to empty (no workspace color rules). Set in hydrix-config to add per-workspace
  # active border colors (e.g. to visually distinguish VM workspaces).
  ...
  # Lock screen idle timeout (seconds) - can be overridden per machine
  idleTimeout = config.hydrix.graphical.lockscreen.idleTimeout or 600;
...
in
  lib.mkIf (cfg.enable && config.hydrix.hyprland.enable) {
    ...
    # ── Lock Screen (hypridle + hyprlock) ────────────────────────────────────
    # services.hypridle = {  # Disabled — option not available in current nixpkgs
    #   enable = true;
    #   settings = {
    #     general = {
    #       lock_cmd = "pidof hyprlock || hyprlock";
    #       before_sleep_cmd = "hyprlock";
    #       lock_cmd_after_sleep = "hyprlock";
    #     };
    #
    #     listener = [
    #       {
    #         # Dim screen after 3 min idle
    #         timeout = 180;
    #         on-timeout = "brightnessctl -d *s set 5%";
    #         on-resume = "brightnessctl -d *s set 100%";
    #       }
    #       {
    #         # Lock screen after 10 min idle
    #         timeout = idleTimeout;
    #         on-timeout = "hyprlock";
    #       }
    #       {
    #         # Suspend after 30 min idle
    #         timeout = 1800;
    ...
    home-manager.users.${username} = {
      ...
      wayland.windowManager.hyprland = {
        ...
        settings = {
          ...
          general = {
            gaps_in = gaps;
            gaps_out = gaps; # Same as inner for consistent spacing
          ...
          animations = {
            ...
            bezier = [
              ...
              "easeIn,  0.05, 0.9, 0.1, 0.9"
              "easeInOut, 0.0, 0.0, 0.2, 1.0"
          ...
          # ── Keybindings ──────────────────────────────────────────────────────
          # User overrides in shared/hyprland.nix extend this via lib.mkOptionDefault
          ...
          # ── Window rules ──────────────────────────────────────────────────────
          # Window rules extendible by user config
          windowrulev2 = lib.mkOptionDefault (
            [
              "float, class:^(pavucontrol)$"
              "float, class:^(lxappearance)$"
              "float, class:^(nm-connection-editor)$"
            ]
            # Workspace-specific border colors — generated from hydrix.hyprland.workspaceColors
            ++ lib.mapAttrsToList (ws: color:
              "col.active_border = rgba(${color}), workspace ${ws}"
            ) workspaceColors
          );
          ...
          exec-once = [
            ...
            # Hyprland colorscheme reload on wal color change
            "sh -c '[ -f ~/.cache/wal/colors.sh ] && source ~/.cache/wal/colors.sh && hyprctl reload 2>/dev/null || true'"
          ];
        };
