...
# The display-setup script substitutes these at runtime based on detected DPI.
{
  ...
  polybarFontFlags = config.hydrix.graphical.font.polybarFontFlags;
  wsLeft = "xworkspaces${
  ...
  # ============================================================
  # VM METRICS POLLER DAEMON
  # Runs as a systemd user service; polls current workspace VM every hostPollInterval seconds.
  # Writes per-VM files to /tmp/hydrix-metrics-<profile> and maintains a
  # /tmp/hydrix-metrics-current symlink pointing to the active VM's file.
  # Polybar modules read only from /tmp/hydrix-metrics-current — no vsock in polybar.
  # ============================================================
  ...
  vmPollerScript = pkgs.writeShellScript "hydrix-vm-poller" ''
    ...
    _get_workspace() {
      ${
        if isSway
        then ''${pkgs.sway}/bin/swaymsg -t get_workspaces 2>/dev/null \
            | ${jq} -r '.[] | select(.focused==true) | .num' \
            | ${head} -1''
        else ''${i3msg} -t get_workspaces 2>/dev/null \
            | ${jq} -r '.[] | select(.focused==true) | .num' \
            | ${head} -1''
      }
    }
    ...
    while true; do
      ...
      if [ -z "$ws" ] || ! printf '%s' "$ws" | ${pkgs.gnugrep}/bin/grep -qE '^[0-9]+$'; then
        ...
        ${pkgs.coreutils}/bin/sleep ${toString hostPollInterval}
        continue
      fi

      profile=$(${jq} -r --argjson w "$ws" \
        'to_entries[] | select(.value.workspace == $w) | .key' \
        "$VM_REGISTRY" 2>/dev/null | ${head} -1)
      cid=$(${jq} -r --argjson w "$ws" \
        'to_entries[] | select(.value.workspace == $w) | .value.cid' \
        "$VM_REGISTRY" 2>/dev/null | ${head} -1)
      vm_name=$(${jq} -r --argjson w "$ws" \
      ...
      response=$(echo "all" | ${timeout} 1 ${socat} - "VSOCK-CONNECT:$cid:14501" 2>/dev/null)
      if [ -n "$response" ]; then
        { printf 'ws=%s\ncid=%s\nvm_name=%s\nvm_online=1\n' "$ws" "$cid" "$vm_name"
          printf '%s\n' "$response"
          printf 'updated=%s\n' "$now"
  ...
  getColorHelper = ''
    if [ -f /tmp/hydrix-colors.sh ]; then
      . /tmp/hydrix-colors.sh
      color_normal="''${color4:-#5e81ac}"
      color_alert="''${color1:-#bf616a}"
      color_prefix="''${color3:-#ebcb8b}"
    else
      color_normal="#5e81ac"
      color_alert="#bf616a"
      color_prefix="#ebcb8b"
  ...
  workspaceDescScript = pkgs.writeShellScript "polybar-workspace-desc" ''
        # Get current workspace number (work with both i3 and sway)
        if command -v ${pkgs.sway}/bin/swaymsg >/dev/null 2>&1 && ${pkgs.procps}/bin/pgrep -x sway >/dev/null 2>&1; then
          ws=$(${pkgs.sway}/bin/swaymsg -t get_workspaces 2>/dev/null | ${pkgs.jq}/bin/jq -r '.[] | select(.focused==true) | .num' | head -1)
        else
  ...
  # CPROC: Most CPU-intensive process in current workspace's VM
  guestCpuBottomScript = pkgs.writeShellScript "polybar-guest-cpu-bottom" ''
    ${_vmCacheHeader "CPROC" "---"}
    top_val=$(${grep} '^top=' "$CACHE" | ${awk} -F= '{print $2}')
    [ -z "$top_val" ] && _stale
    proc_name=$(echo "$top_val" | ${cut} -d' ' -f1 | ${tr} '[:lower:]' '[:upper:]' | ${cut} -c1-28)
    proc_cpu=$(echo "$top_val" | ${cut} -d' ' -f2)
    [ "$proc_cpu" -gt 50 ] 2>/dev/null && uc="$color_alert" || uc="$color_normal"
    echo "%{F$color_prefix}CPROC %{F-}%{u$uc}%{+u}$proc_name $proc_cpu%{-u}"
  '';

  # RPROC: Most RAM-intensive process in current workspace's VM
  guestRamBottomScript = pkgs.writeShellScript "polybar-guest-ram-bottom" ''
    ${_vmCacheHeader "RPROC" "---"}
    top_val=$(${grep} '^topmem=' "$CACHE" | ${awk} -F= '{print $2}')
    [ -z "$top_val" ] && _stale
    proc_name=$(echo "$top_val" | ${cut} -d' ' -f1 | ${tr} '[:lower:]' '[:upper:]' | ${cut} -c1-28)
    proc_ram=$(echo "$top_val" | ${cut} -d' ' -f2)
    [ "$proc_ram" -gt 2048 ] 2>/dev/null && uc="$color_alert" || uc="$color_normal"
    echo "%{F$color_prefix}RPROC %{F-}%{u$uc}%{+u}$proc_name $proc_ram%{-u}"
  '';

  # Cache-reading helper (inlined into each script). All VM bottom modules are pure
  # file readers — the hydrix-vm-poller daemon writes /tmp/hydrix-metrics-current.
  _vmCacheHeader = label: offline: ''
    ${getColorHelper}
    CACHE="/tmp/hydrix-metrics-current"
    STALE=${toString staleThreshold}
    _stale() { echo "%{F$color_prefix}${label} %{F-}%{u$color_normal}%{+u}...%{-u}"; exit 0; }
    _offline() { echo "%{F$color_prefix}${label} %{F-}%{u$color_normal}%{+u}${offline}%{-u}"; exit 0; }
    [ -f "$CACHE" ] || { echo ""; exit 0; }
    updated=$(${grep} '^updated=' "$CACHE" | ${awk} -F= '{print $2}')
    now=$(${pkgs.coreutils}/bin/date +%s)
    [ -z "$updated" ] || [ $((now - updated)) -gt $STALE ] && _stale
    ws=$(${grep} '^ws=' "$CACHE" | ${awk} -F= '{print $2}')
    [ -z "$ws" ] && echo "" && exit 0
    vm_online=$(${grep} '^vm_online=' "$CACHE" | ${awk} -F= '{print $2}')
    [ "$vm_online" != "1" ] && _offline
  '';

  # VM CPU usage — pure cache reader; hydrix-vm-poller daemon handles all fetching.
  vmCpuBottomScript = pkgs.writeShellScript "polybar-vm-cpu-bottom" ''
    ${_vmCacheHeader "CPU" "○"}
    cpu=$(${grep} '^cpu=' "$CACHE" | ${awk} -F= '{print $2}')
    [ -z "$cpu" ] && _stale
    [ "$cpu" -gt 75 ] 2>/dev/null && uc="$color_alert" || uc="$color_normal"
    echo "%{F$color_prefix}CPU %{F-}%{u$uc}%{+u}$cpu%{-u}"
  '';
  ...
  vmRamBottomScript = pkgs.writeShellScript "polybar-vm-ram-bottom" ''
    ...
    [ -z "$ram" ] && _stale
    [ "$ram" -gt 75 ] 2>/dev/null && uc="$color_alert" || uc="$color_normal"
    echo "%{F$color_prefix}RAM %{F-}%{u$uc}%{+u}$ram%{-u}"
  '';
  ...
  vmFsBottomScript = pkgs.writeShellScript "polybar-vm-fs-bottom" ''
    ...
    [ -z "$fs" ] && _stale
    [ "$fs" -gt 75 ] 2>/dev/null && uc="$color_alert" || uc="$color_normal"
    echo "%{F$color_prefix}FS %{F-}%{u$uc}%{+u}$fs%{-u}"
  '';
  ...
  vmUpBottomScript = pkgs.writeShellScript "polybar-vm-up-bottom" ''
    ...
    [ -z "$up" ] && _stale
    echo "%{F$color_prefix}UP %{F-}%{u$color_normal}%{+u}$up%{-u}"
  '';
  ...
  vmSyncDevBottomScript = pkgs.writeShellScript "polybar-vm-sync-dev-bottom" ''
    ...
    [ -z "$dev" ] && _stale
    echo "%{F$color_prefix}DEV %{F-}%{u$color_normal}%{+u}$dev%{-u}"
  '';
  ...
  vmSyncStgBottomScript = pkgs.writeShellScript "polybar-vm-sync-stg-bottom" ''
    ...
    [ -z "$stg" ] && _stale
    [ "$stg" -gt 0 ] 2>/dev/null && uc="$color_alert" || uc="$color_normal"
  ...
  wifiSyncBottomScript = pkgs.writeShellScript "polybar-wifi-sync-bottom" ''
    ...
    # Detect WM and get workspace accordingly
    if command -v ${pkgs.sway}/bin/swaymsg >/dev/null 2>&1 && ${pkgs.procps}/bin/pgrep -x sway >/dev/null 2>&1; then
      current_ws=$(${pkgs.sway}/bin/swaymsg -t get_workspaces 2>/dev/null | ${jq} '.[] | select(.focused==true) | .num' | head -1)
    else
      current_ws=$(${pkgs.i3}/bin/i3-msg -t get_workspaces 2>/dev/null | ${jq} '.[] | select(.focused==true) | .num' | head -1)
    fi
  ...
  # VM TUN: Active VPN/tun interfaces (hides when offline or no VM)
  vmTunBottomScript = pkgs.writeShellScript "polybar-vm-tun-bottom" ''
    ...
    updated=$(${grep} '^updated=' "$CACHE" | ${awk} -F= '{print $2}')
    now=$(${pkgs.coreutils}/bin/date +%s)
    [ -z "$updated" ] || [ $((now - updated)) -gt $STALE ] && echo "" && exit 0
    ws=$(${grep} '^ws=' "$CACHE" | ${awk} -F= '{print $2}')
    [ -z "$ws" ] && echo "" && exit 0
    vm_online=$(${grep} '^vm_online=' "$CACHE" | ${awk} -F= '{print $2}')
    [ "$vm_online" != "1" ] && echo "" && exit 0
    tun=$(${grep} '^tun=' "$CACHE" | ${awk} -F= '{print $2}')
    [ -n "$tun" ] && [ "$tun" != "none" ] \
      && echo "%{F$color_prefix}TUN %{F-}%{u$color_normal}%{+u}$tun%{-u}" \
      || echo ""
  '';
  ...
  unibarTemplate = ''
    ...
    wm-restack = ${if isSway then "none" else "i3"}
    override-redirect = ${
      if floatingBar
      then "true"
      else "false"
    ...
    font-0 = @@FONT_NAME@@${polybarFontFlags}:style=Bold:size=@@FONT_SIZE@@;0
    font-1 = @@FONT_NAME@@${polybarFontFlags}:style=Regular:size=@@FONT_SIZE@@;0
  ...
  modularTemplate = ''
    ...
    font-0 = @@FONT_NAME@@${polybarFontFlags}:style=Bold:size=@@FONT_SIZE@@;3
  ...
  commonModules = ''
    ...
    interval = ${toString hostPollInterval}
    format-background = ''${colors.module-bg}
    format-padding = 1
    label = %output%
  ...
  '';
in {
  config = lib.mkIf (config.hydrix.graphical.enable && (config.hydrix.i3.enable || config.hydrix.sway.enable)) {
    home-manager.users.${username} = {pkgs, ...}: {
      # Install polybar package but DON'T use services.polybar
      # display-setup handles startup with runtime DPI scaling
      home.packages = [
        (pkgs.polybar.override {
          i3Support = true;  # Works for both i3 and sway (same IPC protocol)
          pulseSupport = true;
        })
      ];
