...
# - Creates a systemd user service that generates config then runs dunst
{
  ...
  generateDunstrc = pkgs.writeShellScript "generate-dunstrc" ''
        ...
        # Extract colors from wal cache (works on both host and VMs).
        # xrdb is only populated on the host after walrgb runs; VMs only have colors.json.
        ...
        if [ -f "$WAL_COLORS" ]; then
          ...
          FRAME=$(${pkgs.jq}/bin/jq -r '.colors.color4' "$WAL_COLORS")
