...
# Also regenerates dunst config from scaling.json so notifications track polybar position
{
  ...
  polybarFontFlags = cfg.font.polybarFontFlags;
  ...
  displaySetupScript = pkgs.writeShellScript "display-setup" ''
    ...
    FONT_FLAGS_POLYBAR="${polybarFontFlags}"

    # Configured internal resolution for standalone mode
    INTERNAL_RES="${
      if cfg.scaling.internalResolution != null
      then cfg.scaling.internalResolution
      else ""
    }"
    ...
    position_monitors() {
        ...
        # Save current xrandr gamma/brightness settings for external monitors before repositioning
        # This preserves manual brightness adjustments made with hydrix-brightness
        local GAMMA_SAVE_FILE="/tmp/xrandr-gamma-save"
        > "$GAMMA_SAVE_FILE"
        for ext in $externals; do
            # Extract current gamma/brightness for this output
            local gamma=$(${xrandr} --verbose --query 2>/dev/null | ${awk} -v m="$ext" '
                BEGIN { found=0; gamma="" }
                $1 == m { found=1 }
            ...
            ')
            if [ -n "$gamma" ]; then
                echo "$ext=$gamma" >> "$GAMMA_SAVE_FILE"
            fi
        done
        ...
        # Restore saved gamma/brightness settings for external monitors after positioning
        # This preserves manual brightness adjustments made with hydrix-brightness
        if [ -f "$GAMMA_SAVE_FILE" ] && [ -s "$GAMMA_SAVE_FILE" ]; then
            ${sleep} 0.5
            while IFS='=' read -r monitor gamma; do
                if [ -n "$monitor" ] && [ -n "$gamma" ]; then
                    # Apply gamma with --gamma (format R:G:B)
                    ${xrandr} --output "$monitor" --gamma "$gamma" 2>/dev/null || true
                    echo "$(${date}): Restored gamma for $monitor: $gamma" >> "$LOG"
                fi
    ...
    recover_workspaces() {
        ...
        # Workspace state file for persistence across rebuilds
        local WORKSPACE_STATE_FILE="$HOME/.config/hydrix/workspace-state.json"

        # Get all active outputs (for tracking workspace assignments)
        local all_active_outputs=""
        all_active_outputs=$(${timeout} 2 ${i3msg} -t get_outputs 2>/dev/null | ${jq} -r '.[] | select(.active == true) | .name' 2>/dev/null || true)

        # Save workspace state if monitors are stable (all workspaces have valid outputs)
        # This preserves workspace-to-monitor assignments across rebuilds
        if [ -n "$all_active_outputs" ]; then
            local current_state=$(${timeout} 2 ${i3msg} -t get_workspaces 2>/dev/null | ${jq} -c '.[] | {name, output}' 2>/dev/null || true)
            if [ -n "$current_state" ]; then
                echo "$(${date}): Saving workspace state: $current_state" >> "$LOG"
                echo "$current_state" > "$WORKSPACE_STATE_FILE"
            fi
        fi

        # Restore workspace-to-monitor assignments from saved state when monitors reconnect
        # Only do this if we have a saved state and monitors are stable
        if [ -f "$WORKSPACE_STATE_FILE" ] && [ -s "$WORKSPACE_STATE_FILE" ]; then
            local saved_state
            saved_state=$(cat "$WORKSPACE_STATE_FILE" 2>/dev/null || true)
            if [ -n "$saved_state" ]; then
                echo "$(${date}): Checking saved workspace state for restoration..." >> "$LOG"
                # For each saved workspace/output pair, if workspace exists but is on wrong output, move it back
                echo "$saved_state" | ${jq} -r '.[] | "\(.name)|\(.output)"' 2>/dev/null | while IFS='|' read ws expected_output; do
                    if [ -n "$ws" ] && [ -n "$expected_output" ]; then
                        # Check if this workspace exists and what output it's currently on
                        local current_output
                        current_output=$(${timeout} 2 ${i3msg} -t get_workspaces 2>/dev/null | ${jq} -r --arg ws "$ws" '.[] | select(.name == $ws) | .output' 2>/dev/null || echo "")
                        if [ -n "$current_output" ] && [ "$current_output" != "$expected_output" ]; then
                            # Workspace exists but is on wrong output - check if expected output is active
                            if echo "$active" | ${grep} -q "^$expected_output$"; then
                                echo "$(${date}): Restoring workspace $ws to expected output $expected_output" >> "$LOG"
                                ${timeout} 5 ${i3msg} "workspace $ws; move workspace to output $expected_output" >> "$LOG" 2>&1 || true
                            fi
                        fi
                    fi
                done
    ...
    restart_polybar() {
      ...
      then ''
        ...
        if [ -f "$POLYBAR_TEMPLATE" ]; then
            ...
            ${sed} \
                ...
                -e "s/@@FONT_FLAGS@@/$FONT_FLAGS_POLYBAR/g" \
      ...
      ''
        ...
        # Update i3/sway config FIRST, before launching polybar
        ...
        # Detect if running under sway at runtime
        if ${pkgs.procps}/bin/pgrep -x sway >/dev/null 2>&1; then
            # Update sway config
            if [ -f "$sway_config" ]; then
                ${pkgs.gnused}/bin/sed -i "s/^gaps inner .*/gaps inner $inner_gaps/" "$sway_config"
                ${pkgs.gnused}/bin/sed -i "s/^gaps outer .*/gaps outer $outer_gaps/" "$sway_config"
                echo "$(${date}): Updated sway config: gaps inner=$inner_gaps outer=$outer_gaps" >> "$LOG"
                ${lib.optionalString floatingBar ''
                  ...
                  ${pkgs.gnused}/bin/sed -i "s/^gaps top .*/gaps top $top_gap/" "$sway_config"
                  echo "$(${date}): Updated sway config: gaps top=$top_gap (bar_edge=$bar_edge)" >> "$LOG"
                ''}
                ${lib.optionalString (floatingBar && bottomBar) ''
                  local bar_edge=''${BAR_EDGE_GAPS:-$bar_gaps}
                  local bottom_gap=$((bar_edge + bar_height))
                  ${pkgs.gnused}/bin/sed -i "s/^gaps bottom .*/gaps bottom $bottom_gap/" "$sway_config"
                  echo "$(${date}): Updated sway config: gaps bottom=$bottom_gap" >> "$LOG"
                ''}
                ${lib.optionalString (floatingBar && !bottomBar) ''
                  ${pkgs.gnused}/bin/sed -i "s/^gaps bottom .*/gaps bottom 0/" "$sway_config"
                  echo "$(${date}): Updated sway config: gaps bottom=0 (bottom bar disabled)" >> "$LOG"
                ''}
            fi
            # Reload sway to apply gaps
            if command -v swaymsg >/dev/null 2>&1; then
                ${timeout} 5 swaymsg reload >> "$LOG" 2>&1 || true
                echo "$(${date}): Reloaded sway config" >> "$LOG"
            fi
        else
            # Update i3 config
            if [ -f "$i3_config" ]; then
        ...
        if [ -f "$POLYBAR_TEMPLATE" ]; then
            ...
            for monitor in $monitors; do
                echo "$(${date}): Launching polybar on $monitor (main${
          if bottomBar
          then " + bottom"
          else ""
        })" >> "$LOG"
