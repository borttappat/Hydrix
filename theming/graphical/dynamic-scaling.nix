...
let
  ...
  hydrixScaleScript = pkgs.writeShellScriptBin "hydrix-scale" ''
    ...
    POLYBAR_FONT_FLAGS="${polybarFontFlags}"
    ...
    calculate_values() {
        ...
        REL_WOFI=$(get_effective_relation "wofi")
        ...
        FAMILY_WOFI=$(get_app_family "wofi")
        BASE_WOFI=$(get_family_base_size "$FAMILY_WOFI")
        ...
        OVR_WOFI=$(get_font_override "wofi")
        ...
        FONT_WOFI=''${OVR_WOFI:-$(round "$(LC_ALL=C echo "$BASE_WOFI * $FONT_SCALE * $REL_WOFI" | $BC)")}
        ...
        FONT_WOFI=$(clamp_font_max "$FONT_WOFI" "$(get_font_max wofi)")
        ...
        echo "Fonts: alacritty=$FONT_ALACRITTY polybar=$FONT_POLYBAR rofi=$FONT_ROFI wofi=$FONT_WOFI dunst=$FONT_DUNST"
...
EOF
    ...
    generate_configs() {
        ...
        $SED -e "s/@@FONT_SIZE@@/$FONT_POLYBAR/g" -e "s/@@FONT_NAME@@/$POLYBAR_FONT/g" \
            -e "s/@@FONT_FLAGS@@/$POLYBAR_FONT_FLAGS/g" \
...
in {
  ...
  config = lib.mkIf (cfg.enable && cfg.scaling.auto && (config.hydrix.i3.enable or false)) {
    ...
    # Patch scaling.json on rebuild so font_name and font_names reflect current config.
    # dynamic-scaling only regenerates scaling.json on display events, so without this
    # patch a font change wouldn't take effect until the next display hotplug/login.
    # Per-app font_names entries are reset to the global font unless the user has an
    # explicit familyOverrides entry for that app.
    system.activationScripts.updateScalingFont = lib.mkIf (!isVM) {
      text = ''
        ...
        if [ -f "$SCALING_JSON" ]; then
          ${pkgs.jq}/bin/jq \
            --arg font "${cfg.font.family}" \
            --argjson overrides '${builtins.toJSON cfg.font.familyOverrides}' \
            '
              .font_name = $font |
              .font_names = (.font_names | with_entries(
