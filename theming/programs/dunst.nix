...
# clearance from a tiled window's edge, in principle - ui.dunstOffsetCompensation.x/y
# is a per-machine fudge on top of that for fractional-scale rounding drift (see its
# option description), not part of the base calculation.
#
# This module:
...
# - Creates a systemd user service that runs dunst against both config files
{
  ...
  generateDunstLayout = pkgs.writeShellScript "generate-dunstrc-layout" ''
    ...
    COMP_X=${toString cfg.ui.dunstOffsetCompensation.x}
    COMP_Y=${toString cfg.ui.dunstOffsetCompensation.y}
    ...
    # Offset (see module header): X = gaps + dunstOffset, Y = dunstOffset, both
    # plus the machine's empirical rounding-drift compensation (usually 0).
    OFFSET_X=$((GAPS + DUNST_OFFSET + COMP_X))
    OFFSET_Y=$((DUNST_OFFSET + COMP_Y))
