...
EOF
    ...
    # Get current focused workspace number (WM-aware)
    get_workspace() {
        if [[ -n "''${WAYLAND_DISPLAY:-}" ]] && command -v swaymsg >/dev/null 2>&1; then
            swaymsg -t get_workspaces | ${pkgs.jq}/bin/jq '.[] | select(.focused).num'
        else
    ...
    main() {
        ...
        # Wayland: forward via waypipe (lazy — start tunnel if needed, push mode, send command)
        if [[ -n "''${WAYLAND_DISPLAY:-}" ]] && command -v waypipe-connect >/dev/null 2>&1; then
            local VM_REGISTRY="/etc/hydrix/vm-registry.json"
            local cid
            cid=$(${pkgs.jq}/bin/jq -r --arg v "$vm_name" \
                'to_entries[] | select(.value.vmName == $v) | .value.cid' \
                "$VM_REGISTRY" 2>/dev/null | head -1)
            if [[ -z "$cid" ]]; then
                error "$vm_name not in vm-registry — cannot route via waypipe"
            fi
            if ! pgrep -f "waypipe-connect ''${vm_name}$" >/dev/null 2>&1; then
                notify "Connecting waypipe for $vm_name..."
            ...
            for _i in $(seq 1 20); do
                local st
                st=$(printf 'STATUS\n' | ${pkgs.socat}/bin/socat -T3 - \
                    VSOCK-CONNECT:"$cid":14509 2>/dev/null || true)
                [[ "$st" == "waypipe" ]] && { ready=1; break; }
                sleep 1
            done
            [[ "$ready" -eq 0 ]] && error "waypipe not ready in $vm_name after 20s"
            printf '%s\n' "$*" | ${pkgs.socat}/bin/socat -T10 - VSOCK-CONNECT:"$cid":14508
            return
        fi

        # X11/i3: lazily push xpra mode so VM starts xpra-vsock if in neutral state
        local VM_REGISTRY="/etc/hydrix/vm-registry.json"
        local cid
        cid=$(${pkgs.jq}/bin/jq -r --arg v "$vm_name" \
            'to_entries[] | select(.value.vmName == $v) | .value.cid' \
            "$VM_REGISTRY" 2>/dev/null | head -1)
        [[ -n "$cid" ]] && printf 'xpra\n' | ${pkgs.socat}/bin/socat -T5 - \
            VSOCK-CONNECT:"$cid":14509 2>/dev/null || true
